...
class YMLogs:
    def __init__(self, access_token, counter, date1, date2, source='visits'):
    ...
    def request(self, method, request_type, **kwargs):
    ...
    def evaluate(self):
        ...
        evaluate_response = self.request(method, request_type="get", date1=self.date1, date2=self.date2,
                                         fields=self.__fields, source=self.__source).json()
        return evaluate_response

    def logrequest_id(self):
        method = "logrequests/"
        logrequest_id_response = self.request(method, request_type="post", date1=self.date1, date2=self.date2,
                                              fields=self.__fields, source=self.__source).json()
        return logrequest_id_response['log_request']['request_id']

    def logrequests(self, request_id):
        method = f"logrequest/{request_id}"
        logrequests_response = self.request(method, request_type="get").json()
    ...
    def download(self, request_id, parts):
        ...
        for part in parts:
            ...
            download = self.request(method, request_type="get")
            data = self.get_data(download)
